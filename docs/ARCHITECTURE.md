...
## Small edit to existing non-goals

In the current doc, this line exists under version-one non-goals:

```md
- Writable MCP tools.

Leave it there. Add this clarification directly below it:

- Controlled LLM-assisted vault updates are deferred to V3 and must use proposal/approval/apply semantics, not direct writable MCP tools.
Small edit to MCP invariant
...
This keeps the line clean: LLMs can help maintain the vault later, but they never get raw filesystem control.